...
class HippoRAG:
    ...
    def get_top_k_weights(self,
        ...
        # More lenient assertion: allow for cases where some phrases might have zero weight
        # but are still in linking_score_map (e.g., due to rounding or filtering)
        nonzero_count = np.count_nonzero(all_phrase_weights)
        map_length = len(linking_score_map.keys())
        if nonzero_count != map_length:
            # Log warning instead of failing, and filter out zero-weight phrases from linking_score_map
            logger.warning(f"Phrase weights mismatch: {nonzero_count} nonzero weights vs {map_length} phrases in linking_score_map. Filtering zero-weight phrases.")
            # Remove phrases with zero weight from linking_score_map
            linking_score_map = {phrase: score for phrase, score in linking_score_map.items() 
                                if all_phrase_weights[self.node_name_to_vertex_idx.get(
                                    compute_mdhash_id(content=phrase, prefix="entity-"), -1)] > 0}
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            phrase_weights = np.where(number_of_occurs > 0, phrase_weights / number_of_occurs, 0.0)

        for phrase, phrase_id in phrases_and_ids:
            # Skip phrases with None phrase_id (not found in graph)
