...
1. 在任意临时目录克隆 HippoRAG 官方仓库：
   ...
   ```
2. 将克隆下来的仓库中的 `src/hipporag` 文件夹（整个文件夹）复制到本项目的**根目录**下（即与 `src` 同级）。放置后，目录结构如下：
   ```text
   UniDial-EvalKit/
   ├── src/
   │   ├── model/
...
1. 从官方 GitHub 仓库克隆 `MemoryOS` 项目：
   ```bash
   git clone https://github.com/MemoryOS-Dev/MemoryOS.git
   ```
2. 将克隆下来的项目中 `memoryos-pypi` 文件夹内的内容（包含 `memoryos.py`, `retriever.py`, `utils.py` 等），**重命名并移动到**本项目的**根目录**下，并确保该文件夹名为 `memoryos`：
   ```text
   UniDial-EvalKit/
   ├── src/
   │   ├── model/
...
#### 2.2 环境依赖安装
进入你的 `memoryos` 目录，安装它专属的依赖库：
```bash
conda activate uniconv
pip install -r memoryos/requirements.txt
```
...
1. 从对应的 GitHub 仓库（或源码压缩包中）获取 `A-mem` 核心代码。
   ```bash
   # 假设你通过 Git 克隆了原始项目：
   git clone https://github.com/shenye7436/A-mem.git
   ```
2. 将获取到的项目中的核心记忆代码包（即包含 `memory_layer.py` 的文件夹，比如 `A-mem-main` 或者 `A-mem` 下的代码），**重命名为 `AgenticMemory`**，并将其放置于本项目的**根目录**下（与 `src` 同级）：
   ```text
   UniDial-EvalKit/
   ├── src/
   │   ├── model/
...
#### 3.2 环境依赖安装
AMem 的执行依赖于特定的第三方库，请进入 `AgenticMemory` 目录或直接安装其依赖文件：
```bash
conda activate uniconv
pip install -r AgenticMemory/requirements.txt
```
