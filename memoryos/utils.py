...
try:
    ...
    httpx = None  # httpx may not be installed, but it's usually a dependency of openai
...
# ---- OpenAI Client ----
class OpenAIClient:
    def __init__(self, api_key, base_url=None, max_workers=5, timeout=60):
        ...
        if httpx:
            self.client = OpenAI(
                ...
                timeout=httpx.Timeout(timeout, connect=10)  # 60s timeout, 10s connect timeout
            )
        else:
    ...
    def chat_completion(self, model, messages, temperature=0.7, max_completion_tokens=2000, max_retries=3, retry_delay=2):
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"Error calling OpenAI API (attempt {attempt + 1}/{max_retries}): {error_type}: {error_msg}")
                
                # Check if it's an error we should retry
                should_retry = True
                current_retry_delay = retry_delay
                
                # Check error type and adjust retry strategy
                error_lower = error_msg.lower()
                if "rate limit" in error_lower or "429" in error_msg:
                    # Rate limit: use exponential backoff
                    current_retry_delay = min(retry_delay * (2 ** attempt), 60)  # Exponential backoff, max 60 seconds
                    print(f"Rate limit error detected. Waiting {current_retry_delay} seconds before retry...")
                elif "502" in error_msg or "503" in error_msg or "504" in error_msg:
                    # Server errors: retry with delay
                    current_retry_delay = retry_delay * (attempt + 1)  # Linear backoff
                    print(f"Server error detected. Waiting {current_retry_delay} seconds before retry...")
                elif "timeout" in error_lower or "timed out" in error_lower:
                    # Timeout: retry
                    current_retry_delay = retry_delay
                    print(f"Timeout error detected. Waiting {current_retry_delay} seconds before retry...")
                elif "401" in error_msg or "unauthorized" in error_lower or "invalid api key" in error_lower:
                    # Authentication errors: don't retry
                    should_retry = False
                    print(f"Authentication error detected. Not retrying.")
                elif "400" in error_msg and "bad request" in error_lower:
                    # Bad request errors: don't retry (likely invalid input)
                    should_retry = False
                    print(f"Bad request error detected. Not retrying.")
                else:
                    # Other errors: retry with default delay
                    current_retry_delay = retry_delay
                    print(f"Error detected. Waiting {current_retry_delay} seconds before retry...")
                
                # If this is the last attempt or shouldn't retry, return error
                if attempt == max_retries - 1 or not should_retry:
                    print(f"Failed to get response from LLM after {attempt + 1} attempt(s). Error: {error_msg}")
                    return f"Error: Could not get response from LLM. {error_msg}"
    ...
    def chat_completion_async(self, model, messages, temperature=0.7, max_completion_tokens=2000):
        """异步版本的chat_completion"""
        return self.executor.submit(self.chat_completion, model, messages, temperature, max_completion_tokens)

    def batch_chat_completion(self, requests):
        """
        并行处理多个LLM请求
        requests: List of dict with keys: model, messages, temperature, max_completion_tokens
        """
        futures = []
        for req in requests:
            future = self.chat_completion_async(
                ...
                max_completion_tokens=req.get("max_completion_tokens", 2000)
            )
...
def gpt_summarize_dialogs(dialogs, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    return client.chat_completion(model=model, messages=messages, max_completion_tokens=1000)

def extract_json_from_code_block(text):
    """
    Extract JSON content from code block markers like ```json ... ``` or ``` ... ```
    """
    if not text:
        return text
...
def gpt_generate_multi_summary(text, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    response_text = client.chat_completion(model=model, messages=messages, max_completion_tokens=1000)
    try:
        # Extract JSON from code blocks if present
        json_text = extract_json_from_code_block(response_text)
        summaries = json.loads(json_text)
    except json.JSONDecodeError as e:
        print(f"Warning: Could not parse multi-summary JSON: {response_text[:200]}...")
...
def gpt_user_profile_analysis(dialogs, client: OpenAIClient, model="gpt-4o-mini", existing_user_profile="None"):
    ...
    result_text = client.chat_completion(model=model, messages=messages, max_completion_tokens=1500)
...
def gpt_knowledge_extraction(dialogs, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    result_text = client.chat_completion(model=model, messages=messages, max_completion_tokens=2000)
...
def gpt_extract_theme(answer_text, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    return client.chat_completion(model=model, messages=messages, max_completion_tokens=100)
...
def check_conversation_continuity(previous_page, current_page, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    response = client.chat_completion(model=model, messages=messages, temperature=0.0, max_completion_tokens=10)
...
def generate_page_meta_info(last_page_meta, current_page, client: OpenAIClient, model="gpt-4o-mini"):
    ...
    return client.chat_completion(model=model, messages=messages, temperature=0.3, max_completion_tokens=100).strip()