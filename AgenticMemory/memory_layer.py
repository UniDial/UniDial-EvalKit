...
class OpenAIController(BaseLLMController):
    ...
    def get_completion(self, prompt: str, response_format: dict = None, temperature: float = 0.7) -> str:
        # 根据 response_format 决定 system 指令（否则你会要求 JSON 但又让它返回 text）
        if isinstance(response_format, dict) and response_format.get("type") in ("json_object", "json_schema"):
            system_content = "You must respond with a JSON object."
        else:
        ...
        # 先尝试带 response_format（OpenAI 官方支持；部分代理/DeepSeek 可能不支持）
        ...
        try:
            ...
            # DeepSeek/某些代理会报：This response_format type is unavailable now
            msg = str(e)
            if ("response_format" in msg) or ("unavailable now" in msg) or ("invalid_request_error" in msg):
...
class LLMController:
    ...
    def __init__(self, 
        ...
        if backend == "openai":
            self.llm = OpenAIController(model, api_key, api_base=api_base)
        elif backend == "ollama":
    ...
    def find_related_memories_raw(self, query: str, k: int = 5) -> List[MemoryNote]:
        ...
        for i in indices:
            ...
            for neighbor in neighborhood:
                # Convert neighbor to integer, handling different formats
                neighbor_idx = None
                if isinstance(neighbor, int):
                    neighbor_idx = neighbor
                elif isinstance(neighbor, str):
                    # Handle formats like "memory index:3" or just "3"
                    if "memory index:" in neighbor.lower():
                        # Extract number after "memory index:"
                        try:
                            neighbor_idx = int(neighbor.split(":")[-1].strip())
                ...
                # Check if neighbor_idx is a valid index
                if neighbor_idx is not None and isinstance(neighbor_idx, int) and 0 <= neighbor_idx < len(all_memories):
